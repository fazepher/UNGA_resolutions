...
fit_topic_model <- function(model, k, 
                            data = resol_text_df, 
                            text_field = "Resol_Text_WPOP", docid_field = "Resol_Text_WPOP",
  ...
  tokenized_unga <- corpus_unga %>% 
    tokens(remove_punct = remove_punct, 
           ...
           padding = T) %>% 
    tokens_remove(pattern = en_stopwords, padding = T) 
  
  if(remove_1_2l_words){
    tokenized_unga <- tokenized_unga %>% 
      tokens_remove(pattern = "^[A-z]{1,2}$")
  }
...
top_terms <- lda_unga[[1]] %>%
  map_dfr(~ get_terms(.x,50) %>%
            as_tibble() %>%
            {mutate(.,k = ncol(.), rank = row_number())} %>%
            gather(Topic,Word,-k,-rank) %>%
            mutate(Model = "LDA"))

top_terms <- ctm_unga[[1]] %>%
  map_dfr(~ get_terms(.x,30) %>%
            as_tibble() %>%
            {mutate(.,k = ncol(.), rank = row_number())} %>%
            gather(Topic,Word,-k,-rank) %>%
            mutate(Model = "CTM")) %>%
...
top_terms %>%
  filter(k == 9, Model == "LDA") %>%
  ggplot(aes(x=Topic,y=rank,label=stringr::str_wrap(Word,20))) +
  facet_wrap(~Topic, nrow = 2, scales = "free") +
  geom_text(aes(size = nchar(Word) >= 20)) +
  scale_size_manual(values = c(rel(3),rel(1.75))) +
  scale_y_reverse() + 
  theme_classic()

# lda_unga_orig <- lda_unga
# ctm_unga_orig <- ctm_unga
# top_terms_orig <- top_terms
